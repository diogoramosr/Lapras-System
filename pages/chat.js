        ...
        <div className="py-5 flex flex-col gap-2 justify-center items-center bg-slate-600 ">
            {!chatvisivel 
                ...
                <>
                    ...
                    <button className="pl-5" onClick={entrarNaSala}>Entrar</button>
                </>

                :

                <>
                    <h5>Sala : { sala } || Nome : { nome }</h5>
                    <div className="h-52 w-64 border-solid border-x-stone-50 mb-3 py-3">
                        {mensagens.map(el => <div key={v4()}>{el}</div>)}
                    </div>

                    <input type='text' placeholder="Mensagem" value={novaMensagem} onChange={ e => setnovaMensagem(e.target.value)}/>
